    ...
    return (
        <section className="add-item-car">
            <form className="add-item-car_form" onSubmit={sendCarItem} action="" method="" encType='multipart/form-data'>
                <input ref={myRef} className="add-item-car_input" onChange={(e) => onValueInputChange(e)} value={title} placeholder="Марка/модель автомобиля" required name="title" type="text" />
                ...
                <p className="selectTypeCar">
                    ...
                    <select id="type-car" className="form-select" aria-label="Default select example" name="typeCar" value={typeCar} onChange={(e) => onValueInputChange(e)}>
